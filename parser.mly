...
AType :
  ...
  | NAT
      { fun ctx -> TyNat }
  | TENSOR LSQUARE ilist = separated_list(COMMA, INTV) RSQUARE
      { fun ctx -> TyTensor(List.map (fun ii -> ii.v) ilist)}
...
AppTerm :
  ...
  | PathTerm PLUS PathTerm
      { fun ctx -> TmAdd($2, $1 ctx, $3 ctx) }
  | PathTerm MINUS PathTerm
      { fun ctx -> TmSub($2, $1 ctx, $3 ctx) }
  | PathTerm STAR PathTerm
      { fun ctx -> TmTimes($2, $1 ctx, $3 ctx) }
  | PathTerm SLASH PathTerm
...
TensorFields :
    flist = separated_nonempty_list(COMMA, FLOATV)
      { Vector (List.map (fun ff -> Scalar ff.v) flist) }
  | vlist = separated_nonempty_list(COMMA, delimited(LSQUARE, TensorFields, RSQUARE))
      { Vector vlist }
