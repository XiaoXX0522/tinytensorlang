...
type term =
  ...
  | TmInert of info * ty
  | TmTensor of info * int list * float array
...
and printtm_ATerm outer ctx t = match t with
  ...
  | TmTensor(fi,shape,data) ->
    let rec pv start len array : unit = 
    ...
    in let rec pt start tshape array : unit = match tshape with 
      ...
      | [vlen] -> 
          pv start vlen array
      | [vnum; vlen] -> 
          pm start tshape array
      | tnum::rest ->
          if tnum = 1 then 
            (pr "["; obox0(); pt start rest array; cbox(); pr "]")
          else
            (pr "["; obox0(); pt start rest array; cbox(); pr "]";
            print_space(); print_space();
            (pt (start+(Tensorhelper.prod rest)) ((tnum-1)::rest) array))
    in pr "["; obox0(); pt 0 shape data ;cbox(); pr "]"
